...
# result per weekdays and months
ggplot(data=food, aes(x=weekdays(date), fill=result)) + geom_bar() 
ggplot(data=food, aes(x=months(date), fill=result)) + geom_bar() 
...
#check also: http://www.r-statistics.com/2012/03/do-more-with-dates-and-times-in-r-with-lubridate-1-1-0/
browseVignettes("lubridate")

food$ldate <- ymd_hms(food$issdttm, tz="EST") 

#this gives us months, days, years as numbers. 
month(food$ldate)
day(food$ldate)
year(food$ldate) > 2010

# show a histogram only starting the year 2009
f10 <- food[year(food$ldate)>2009,]
ggplot(f10,aes(x=ldate)) + geom_histogram()

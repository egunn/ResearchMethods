...
ggplot(data=cutMelt, aes(x=cutMelt$factor, y=cutMelt$value,fill=cutMelt$x2013))+geom_bar(width=.5,stat="identity")+coord_flip()+labs(title="Total and agricultural land area for 50 largest countries", x="Country", y = "Land area (sq. km)")+scale_y_continuous(labels=comma)+ theme(axis.title.x = element_text(size=12),axis.title.y = element_text(size=12), axis.text.x = element_text(size =10), axis.text.y = element_text(size =7), plot.title=element_text(size=16))+scale_fill_manual(values=c("#E69F00","#999999"), name="Land Type", breaks=c("X2013", "absAgricArea"),  labels=c("Agricultural", "Total"))
